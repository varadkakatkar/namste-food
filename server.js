...
const proxyRestaurants = async (req, res) => {
  try {
    ...
    const url = `https://www.swiggy.com/dapi/restaurants/list/v5?lat=${encodeURIComponent(lat)}&lng=${encodeURIComponent(lng)}&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING`;

    const fetchOptions = {
      headers: {
        'User-Agent': req.get('user-agent') || 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36',
    ...
    if (!response.ok) {
      ...
      console.warn('Primary upstream failed with', originalStatus, 'retrying via corsproxy');
      response = await fetch(proxyUrl, fetchOptions);
      if (!response.ok) {
        const text = await response.text().catch(() => '');
        console.error('Upstream error (after proxy)', response.status, text?.slice(0, 300));
        // Final fallback: minimal mock payload to keep UI working in dev
        const fallback = {
          data: {
            cards: [
              { card: { card: { gridElements: { infoWithStyle: { restaurants: [
                { info: { id: 'dev-1', name: 'Mock Restaurant 1', avgRating: 4.5 } },
                { info: { id: 'dev-2', name: 'Mock Restaurant 2', avgRating: 4.2 } }
              ] } } } } }
            ]
          }
        };
    ...
    res.status(200).json({
      data: {
        cards: [
          { card: { card: { gridElements: { infoWithStyle: { restaurants: [
            { info: { id: 'proxy-error-1', name: 'Proxy Error Mock 1', avgRating: 4.1 } },
            { info: { id: 'proxy-error-2', name: 'Proxy Error Mock 2', avgRating: 4.0 } }
          ] } } } } }
        ]
      },
